...
export async function POST(request: NextRequest) {
    try {
        ...
        // Send notification to all Hostel Office users from Firestore
        const hostelOfficeQuery = await adminDb.collection('employees')
            .where('role', '==', 'Hostel Office')
            .get();
        const hostelOfficeUserEmails = hostelOfficeQuery.docs.map(d => (d.data() as any).email).filter(Boolean);
        let successCount = 0;
        let errorCount = 0;
        ...
        for (const hostelOfficeEmail of hostelOfficeUserEmails) {
            try {
                ...
                if (response.ok) {
                    ...
                    if (result.success) {
                        successCount++;
                        console.log(`✅ Hostel Office notification sent to ${hostelOfficeEmail} for escalation #${escalationId}`);
                    } else {
                        errorCount++;
                        console.error(`❌ Failed to send Hostel Office notification to ${hostelOfficeEmail} for escalation #${escalationId}`);
                    }
                } else {
                    errorCount++;
                    console.error(`❌ Failed to send Hostel Office notification to ${hostelOfficeEmail} for escalation #${escalationId}`);
                ...
                errorCount++;
                console.error(`❌ Error sending Hostel Office notification to ${hostelOfficeEmail} for escalation #${escalationId}:`, error);
            }
        }

        return NextResponse.json({
            success: true,
            message: `Hostel Office notifications sent: ${successCount} successful, ${errorCount} failed`,
            totalSent: successCount,
            totalFailed: errorCount
        });

    } catch (error: any) {
        console.error('Error in Hostel Office status update notification API:', error);
        return NextResponse.json(
